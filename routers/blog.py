...
@alok.get("/blog", tags=["General"])
def get_All(db :Session=Depends(get_db)):
    blogs=db.query(Blog).all()
    return blogs

# get with id 
@alok.get("/blog/{id}", status_code=status.HTTP_200_OK, response_model=show, tags=["General"])
...
# post    
@alok.post("/blog", status_code=status.HTTP_201_CREATED, tags=["General"]) 
...
# delete
@alok.delete("/blog/{id}", tags=["General"])
...
# update
@alok.put("/blog/{id}", status_code=status.HTTP_202_ACCEPTED, tags=["General"])
